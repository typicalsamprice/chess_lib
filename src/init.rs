...
fn init_knight_attacks() {
    ...
    Bitboard::MAX.map_by_board(|square| {
        ...
        unsafe { KNIGHT_ATTACKS[square.get_square().inner() as usize] = nnw | nne | nww | nee | ssw | sse | sww | see; }
    });
}
...
fn init_between_square_lines() {
    Bitboard::MAX.map_by_board(|sq1b| {
        Bitboard::MAX.map_by_board(|sq2b| {
            ...
            if s1 == s2 || !s1.in_line(s2) || unsafe { BETWEEN_SQUARES[s1.inner() as usize][s2.inner() as usize].nonzero() } {
                return;
            }
            ...
            loop {
                if shift_left {
                    ...
                    cp >>= total_shift.abs() as u32;
                }

                if (cp & sq2b).nonzero() { break; }
                betw |= cp;
            }

            unsafe { BETWEEN_SQUARES[s1.inner() as usize][s2.inner() as usize] = betw; }
        });
    });
}
